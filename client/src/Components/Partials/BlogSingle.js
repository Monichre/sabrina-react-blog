...
export default class BlogSingle extends Component {
  ...
  handleLinkClick(params) {
    ...
    let category = article.fields.category
      ? article.fields.category[0].fields.title.split(' ')[0].toLowerCase()
      : null
  ...
  render() {
    ...
    const { article } = data
    const the_other_articles = data.articles.filter(
      other_article => other_article != article
    )
    ...
    fashion_obj.articles = _.filter(
      the_other_articles,
      article =>
        article.sys.contentType.sys.id === 'blogPost' &&
        article.fields.category[0].fields.title === 'Fashion Posts'
    )
      .sort((a, b) => {
        return moment.utc(a.sys.createdAt).diff(moment.utc(b.sys.createdAt))
      })
      .reverse()
    health_obj.articles = _.filter(
      the_other_articles,
      article =>
        article.sys.contentType.sys.id === 'blogPost' &&
        article.fields.category[0].fields.title === 'Health Posts'
    )
      .sort((a, b) => {
        return moment.utc(a.sys.createdAt).diff(moment.utc(b.sys.createdAt))
      })
      .reverse()
    travel_obj.articles = _.filter(
      the_other_articles,
      article =>
        article.sys.contentType.sys.id === 'blogPost' &&
        article.fields.category[0].fields.title === 'Travel Posts'
    )
      .sort((a, b) => {
        return moment.utc(a.sys.createdAt).diff(moment.utc(b.sys.createdAt))
      })
          ...
          {allVideos.splice(0, 3).map((video, i) => (
            <li key={'vidThumb-' + i}>
              <div className="thumb">
                <ReactPlayer
                  url={
                    video.fields.videos && video.fields.videos[0].fields
                      ? video.fields.videos[0].fields.file.url
                      : video.fields.link
                  }
                  width="95%"
                  height="100%"
              ...
              <p className="text">
                <Link
                  to={'/videos/' + video.fields.title}
                  onClick={this.handleLinkClick.bind(video.fields.title)}>
                  {video.fields.title}
                </Link>
              </p>
            </li>
    ...
    let created =
      CONSTANTS.months[date_obj.getMonth()] +
      ' ' +
      date_obj.getDate() +
      ', ' +
    ...
    let readMore = (
      <Link
        to={'/' + category + '/' + article.fields.title}
        onClick={this.scrollTop}>
        Read More
      </Link>
    )
    let article_main_images = CONSTANTS.hasPhoto(article)
      ? article.fields.mainPhotos
      : null
    let blog_post_single
    let path = this.props.match.path.split('/')
    let subTitle = Object.keys(article.fields).includes('subHeader')
      ? article.fields.subHeader
      : null
    ...
    if (path.includes('videos')) {
      ...
      let videos =
        Object.keys(article.fields).includes('videos') &&
        article.fields.videos.length > 0
          ? article.fields.videos
          : null
      blog_post_single = (
        <Article
          article={article}
    ...
    return (
      <div id="Blog">
        <Helmet>
          <meta
            property="og:url"
            content={`http://www.theresaonthetown.com${slug}`}
          />
          ...
          <meta
            property="og:image"
            content={
              CONSTANTS.hasPhoto(article)
                ? article.fields.mainPhotos[0].fields.file.url
                : null
            }
          />
        </Helmet>
          ...
          <div className="container">
            <div className="row">
              <div className="col-xs-12">
                <div className="content-wrap">{blog_post_single}</div>
                          ...
                          if (article_list.articles.length > 0) {
                            return (
                              <div className="recent-list-container">
                                <h5 className="widget-category-title">
                                  {article_list.title}:
                                </h5>
                                <ul className="recent-list clearfix">
                                  {article_list.articles
                                    ...
                                    .map((article, i) => {
                                      let category = article.fields.category
                                        ? article.fields.category[0].fields.title
                                            .split(' ')[0]
                                            .toLowerCase()
                                        : null
                                        ...
                                        <li>
                                          <div className="thumb">
                                            <img
                                              src={
                                                article.fields.mainPhotos &&
                                                article.fields.mainPhotos[0]
                                                  .fields
                                                  ? article.fields.mainPhotos[0]
                                                      .fields.file.url +
                                                    '?w=315&h=315&fit=thumb'
                                                  : null
                                              }
                                              alt="image"
                                            />
