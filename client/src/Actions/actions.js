...
export function getStore(callback) {
  ...
  cms_client.getEntries().then(response => {
    ...
    let section_headers = _.filter(
      response_items,
      item => item.sys.contentType.sys.id === 'sectionHeader'
    )
    let polaroids = _.filter(
      response_items,
      item => item.sys.contentType.sys.id === 'polaroid'
    )
    let video_entries = _.filter(
      response_items,
      item => item.sys.contentType.sys.id === 'videoPost'
    )
    let affiliate_entries = _.filter(
      response_items,
      item => item.sys.contentType.sys.id === 'affiliatePost'
    )
    let initial_articles = _.filter(
      response_items,
    ...
    let mailingListCTA = _.find(
      CTAs,
      item => item.fields.section === 'Mailing List'
    )
    let articles = initial_articles
      ...
      .reverse()
    let sorted_videos = video_entries
      .sort((a, b) => {
        return moment.utc(a.sys.createdAt).diff(moment.utc(b.sys.createdAt))
      })
      .reverse()
    ...
    AppStore.data.featured = _.filter(
      articles,
      article => article.fields.featured === true
    )
    AppStore.data.fashion = _.filter(
      articles,
      article => article.fields.category[0].fields.title === 'Fashion Posts'
    )
    AppStore.data.travel = _.filter(
      articles,
      article => article.fields.category[0].fields.title === 'Travel Posts'
    )
    AppStore.data.health = _.filter(
      articles,
      article => article.fields.category[0].fields.title === 'Health Posts'
    )
